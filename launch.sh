...
if ! [[ -f Create-Ultimate%20Selection%20Serverpack%20MC%201.20.1-12.4.0.zip ]]; then
    ...
    curl -Lo Create-Ultimate%20Selection%20Serverpack%20MC%201.20.1-12.4.0.zip 'https://edge.forgecdn.net/files/6641/138/Create-Ultimate%20Selection%20Serverpack%20MC%201.20.1-12.4.0.zip'
    unzip -u -o 'Create-Ultimate%20Selection%20Serverpack%20MC%201.20.1-12.4.0.zip' -d /data
...
if [[ -n "$MOTD" ]]; then
    ...
    sed -i "s/motd\s*=/ c motd=Create Ultimate Selection 12.4.0 Server Powered by Docker" /data/server.properties
fi
